<template>
    <div class="new-backdrop" @click.self="closeModal">
        <div class="new-modal" @click.self="modal=false">
            ...
            <button :disabled="!this.name" @click="makeEmployee">Remove Employee</button>
...
export default {
    methods:{
        ...
        async makeEmployee(customer){
            ...
            if(person.length == 0){
                alert("Cannot remove employee")
                return
            }

            var sql = `UPDATE customers set employee = 0 where id = ${person[0][0]}`
            ...
            alert(`${this.name} is no longer an employee`)
            this.closeModal()
        }
    },
